...
public class Game {
	...
	private static Scanner scanner = new Scanner(System.in); // TODO: Initialize
																// the value of
																// scanner so
																// that you can
	...
	public static void main(String[] args) throws InterruptedException {
		if (Constants.MINER_BACKPACK_SIZE > Constants.MAP_RESOURCES_NUMBER) {
			System.out
					.println("[Game] You bastard! Your miner can carry more than the map can offer!");
			return;
		}
		...
		/**
		 * pt a apela metoda printFileCOntents am creat o instanta a clasei
		 * FileReader
		 */
		...
		// TODO: Read miner name and place it into a variable.. Look around for
		// the variable name

		String minerName;
		minerName = scanner.nextLine();// read next line from keyboard and save
										// it to minerName

		System.out.println("[Game] Miner '" + minerName
		...
		System.out
				.println("[Game] You have "
						+ Constants.NUMBER_OF_ROUNDS
						+ " rounds to find as many valuable resources as possible! Good luck!");
		...
		for (int i = 0; i < Constants.NUMBER_OF_ROUNDS; i++) {
			...
			Thread.sleep(1000);
			System.out.println("[Game] What should " + miner.getName()
					+ " do next? Dig(1) or sleep(0)?");
			...
			if (action == 1) // operatorul == e folosit pt a compara valori
								// primitive
			...
			case 0:
				...
				Thread.sleep(2000);
				// TODO: Make the miner sleep and update the resource variable
				// with the
				// value returned by the sleep method
				miner.sleep(); // apelez metoda sleep()
				valuableResources = miner.sleep();
				break;
			case 1:
				minerSlept = false;
				System.out.println("[Game] GOGOGO! Our miner is digging..");
				Thread.sleep(2000);
				// TODO: Make the miner dig and update the resource variable
				// with the
				// value returned by the sleep method
				miner.dig();
				valuableResources = miner.dig();
				break;
			default:
				resource = 0;
				break;
			...
			if (map.consumeResource(resource)) {
				if (minerSlept) {
					System.out
							.println("[Game] BOO! A valuable resource decayed because you slept!");
				} else {
					valuableResources++;
					System.out
							.println("[Game] JACKPOT! Valuable resource found!");
				}
			} else {
				if (minerSlept) {
					System.out
							.println("[Game] PHEW! You almost dug for a useless resource!");
				} else {
					System.out
							.println("[Game] BADLUCK! Try sleeping next time!");
				}
			}
		...
		System.out.println("[Game] ####################################");
		System.out.println("[Game] GAME OVER! Your miner found "
				+ valuableResources + "/" + Constants.MAP_RESOURCES_NUMBER
				+ " valuable resources");
